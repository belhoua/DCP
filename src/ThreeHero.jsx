...
export default function ThreeHero({ dark = true }) {
  ...
  useEffect(() => {
    ...
    const tick = () => { renderFrame(); raf = requestAnimationFrame(tick) }
    const updateRun = () => {
      if (reduced) return
    ...
    // Only render while the hero is actually on screen (and tab is visible)
    const io = new IntersectionObserver(([e]) => { onScreen = e.isIntersecting; updateRun() }, { threshold: 0 })
    io.observe(mount)
